export const evaluateWinner = (hands, playType, gameState) => {
  ...
  return Object.keys(hands).reduce((a, b) => 
    getHigherHand(hands[a], hands[b], playType, gameState) ? a : b); 
...
export const getHigherHand = (hand1, hand2, playType, gameState) => {
  ...
  switch(playType) {
    ...
    case playTypes.consecutivePair:
      ...
      if (isHand1ConsecutivePair && !isHand2ConsecutivePair) return true;
      if (!isHand1ConsecutivePair && isHand2ConsecutivePair) return false;
